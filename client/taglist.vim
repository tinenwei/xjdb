...
if !exists('loaded_taglist')
    ...
    command! -nargs=0 -bar TlistSetBreakPoint call Tlist_Set_BreakPoint_By_Line()
...
function! s:Tlist_Parse_Tagline(tag_line)
    ...
    if ttype == 'p'
   	let s:tlist_{s:fidx}_tag_name_p=tag_name
    endif
    ...
    if g:Tlist_Display_Prototype
        ...
        if g:Tlist_Display_Tag_Scope
    		...
    		let s:tlist_{s:fidx}_{s:tidx}_tag_name_scope=tag_scope
            endif
        endif
    endif
...
" function! Tlist_Get_BreakString_By_Line(...)
function! Tlist_Set_BreakPoint_By_Line(...)
    ...
    let package=s:tlist_{fidx}_tag_name_p
    "return package.s:tlist_{fidx}_{tidx}_tag_name_scope.":".linenr
    let breakstr=package.".".s:tlist_{fidx}_{tidx}_tag_name_scope.":".linenr
    let breakstr="b ". breakstr
    let send_cmd="java -classpath /usr/lib/xjdb CommandClient"
    let send_cmd=send_cmd . " localhost 6666 " . breakstr
    "echo send_cmd
    "let out=system(send_cmd)
    "return send_cmd
    call system(send_cmd)
endfunction
