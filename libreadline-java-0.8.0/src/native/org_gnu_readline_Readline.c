...
#define	BINDKEY_PAGE_UP		0
#define 	BINDKEY_PAGE_DOWN	1
#define 	BINDKEY_F11			2
#define 	BINDKEY_F12 			3

static jmethodID jniBindKeyMethodId;
static jclass    jniBindKeyClass;
static jobject   jniBindKeyObject;
...
int handlePageUp()
{
	...
	if(!jniBindKeyMethodId)
		jniBindKeyMethodId = (*jniEnv)->GetMethodID(jniEnv, jniBindKeyClass, "handleKey",
				"(I)V");	   


	if(jniBindKeyMethodId == 0)
		return  0;	   

	(*jniEnv)->CallVoidMethod(jniEnv, jniBindKeyObject, jniBindKeyMethodId, BINDKEY_PAGE_UP);		
...
int handlePageDown()
{
	...
	(*jniEnv)->CallVoidMethod(jniEnv, jniBindKeyObject, jniBindKeyMethodId, BINDKEY_PAGE_DOWN);		
...
int handleF11()
{
	...
	(*jniEnv)->CallVoidMethod(jniEnv, jniBindKeyObject, jniBindKeyMethodId, BINDKEY_F11);		
...
{		
	...
	(*jniEnv)->CallVoidMethod(jniEnv, jniBindKeyObject, jniBindKeyMethodId, BINDKEY_F12);		
...
JNIEXPORT jboolean JNICALL Java_org_gnu_readline_Readline_BindKeyImpl
  (JNIEnv *env, jclass theClass, jint key)
{
  // printf("Java_org_gnu_readline_Readline_BindKeyImpl key =%d \n", key);
   rl_command_func_t*  func = 0;

  
   if(key == BINDKEY_PAGE_UP)
   {
   	func  = handlePageUp;
	if (rl_bind_keyseq ("\033[5~", func))
		return (jboolean) JNI_TRUE;
   }
   else if(key == BINDKEY_PAGE_DOWN)
   {
   	func = handlePageDown;
	if (rl_bind_keyseq ("\033[6~", func))
		return (jboolean) JNI_TRUE;
	
   }else if(key == BINDKEY_F11)
   {
   	func = handleF11;
	if (rl_bind_keyseq ("\033[23~", func))
		return (jboolean) JNI_TRUE;
	
   } 
   else if(key == BINDKEY_F12)
   {
   	func = handleF12;
	if (rl_bind_keyseq ("\033[24~", func))
		return (jboolean) JNI_TRUE;
...
JNIEXPORT void JNICALL Java_org_gnu_readline_Readline_setBindKeyImpl
                                      (JNIEnv *env, jclass class, jobject obj) {
  
  if (obj != NULL ) {
    jniBindKeyObject = obj;
    jniBindKeyClass = (*jniEnv)->GetObjectClass(jniEnv, jniBindKeyObject);
    jniBindKeyClass = (*env)->NewGlobalRef(env, jniBindKeyClass);
    jniBindKeyObject = (*env)->NewGlobalRef(env, jniBindKeyObject);
    jniBindKeyMethodId = (*jniEnv)->GetMethodID(jniEnv, jniBindKeyClass, "handleKey",
				"(I)V");
